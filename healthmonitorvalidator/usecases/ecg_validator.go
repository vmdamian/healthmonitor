...
type ECGValidator struct {
	HighMargin float64
	LowMargin  float64
}

func NewECGValidator(lowMargin float64, highMargin float64) *ECGValidator {
	return &ECGValidator{
...
func (tv *ECGValidator) CheckData(dataSet *domain.DeviceDataset) []*domain.Alert {
	...
	for _, dataPoint := range dataSet.Data {
		...
		if dataPoint.ECG >= tv.HighMargin {
			if currentAlert == nil {
				currentAlert = &domain.Alert{
					DID:                 dataSet.DID,
					AlertType:           domain.ALERT_TYPE_ECG_HIGH,
	...
	for _, dataPoint := range dataSet.Data {
		...
		if dataPoint.ECG <= tv.LowMargin {
			if currentAlert == nil {
				currentAlert = &domain.Alert{
					DID:                 dataSet.DID,
					AlertType:           domain.ALERT_TYPE_ECG_LOW,
					Status:              domain.ALERT_STATUS_ACTIVE,
					CreatedTimestamp:    dataPoint.Timestamp,
					LastActiveTimestamp: dataPoint.Timestamp,
				}
